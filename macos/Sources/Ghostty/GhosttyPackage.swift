...
extension Ghostty {
    ...
    static let userNotificationCategory = "info.olaugh.aghoul.userNotification"

    // The user notification "Show" action
    static let userNotificationActionShow = "info.olaugh.aghoul.userNotification.Show"
}
...
extension Notification.Name {
    ...
    static let ghosttyConfigDidChange = Notification.Name("info.olaugh.aghoul.configDidChange")
    static let GhosttyConfigChangeKey = ghosttyConfigDidChange.rawValue

    /// Color change. Object is the surface changing.
    static let ghosttyColorDidChange = Notification.Name("info.olaugh.aghoul.ghosttyColorDidChange")
    static let GhosttyColorChangeKey = ghosttyColorDidChange.rawValue

    /// Goto tab. Has tab index in the userinfo.
    static let ghosttyMoveTab = Notification.Name("info.olaugh.aghoul.moveTab")
    static let GhosttyMoveTabKey = ghosttyMoveTab.rawValue

    /// Close tab
    static let ghosttyCloseTab = Notification.Name("info.olaugh.aghoul.closeTab")

    /// Close other tabs
    static let ghosttyCloseOtherTabs = Notification.Name("info.olaugh.aghoul.closeOtherTabs")

    /// Close tabs to the right of the focused tab
    static let ghosttyCloseTabsOnTheRight = Notification.Name("info.olaugh.aghoul.closeTabsOnTheRight")

    /// Close window
    static let ghosttyCloseWindow = Notification.Name("info.olaugh.aghoul.closeWindow")

    /// Resize the window to a default size.
    static let ghosttyResetWindowSize = Notification.Name("info.olaugh.aghoul.resetWindowSize")

    /// Ring the bell
    static let ghosttyBellDidRing = Notification.Name("info.olaugh.aghoul.ghosttyBellDidRing")

    /// Readonly mode changed
    static let ghosttyDidChangeReadonly = Notification.Name("info.olaugh.aghoul.didChangeReadonly")
    static let ReadonlyKey = ghosttyDidChangeReadonly.rawValue + ".readonly"
    static let ghosttyCommandPaletteDidToggle = Notification.Name("info.olaugh.aghoul.commandPaletteDidToggle")

    /// Toggle maximize of current window
    static let ghosttyMaximizeDidToggle = Notification.Name("info.olaugh.aghoul.maximizeDidToggle")

    /// Notification sent when scrollbar updates
    static let ghosttyDidUpdateScrollbar = Notification.Name("info.olaugh.aghoul.didUpdateScrollbar")
    static let ScrollbarKey = ghosttyDidUpdateScrollbar.rawValue + ".scrollbar"

    /// Focus the search field
    static let ghosttySearchFocus = Notification.Name("info.olaugh.aghoul.searchFocus")
}
...
extension Ghostty.Notification {
    ...
    static let NewSurfaceConfigKey = "info.olaugh.aghoul.newSurfaceConfig"

    /// Posted when a new split is requested. The sending object will be the surface that had focus. The
    /// userdata has one key "direction" with the direction to split to.
    static let ghosttyNewSplit = Notification.Name("info.olaugh.aghoul.newSplit")

    /// Close the calling surface.
    static let ghosttyCloseSurface = Notification.Name("info.olaugh.aghoul.closeSurface")

    /// Focus previous/next split. Has a SplitFocusDirection in the userinfo.
    static let ghosttyFocusSplit = Notification.Name("info.olaugh.aghoul.focusSplit")
    static let SplitDirectionKey = ghosttyFocusSplit.rawValue

    /// Goto tab. Has tab index in the userinfo.
    static let ghosttyGotoTab = Notification.Name("info.olaugh.aghoul.gotoTab")
    static let GotoTabKey = ghosttyGotoTab.rawValue

    /// New tab. Has base surface config requested in userinfo.
    static let ghosttyNewTab = Notification.Name("info.olaugh.aghoul.newTab")

    /// New window. Has base surface config requested in userinfo.
    static let ghosttyNewWindow = Notification.Name("info.olaugh.aghoul.newWindow")

    /// Present terminal. Bring the surface's window to focus without activating the app.
    static let ghosttyPresentTerminal = Notification.Name("info.olaugh.aghoul.presentTerminal")

    /// Toggle fullscreen of current window
    static let ghosttyToggleFullscreen = Notification.Name("info.olaugh.aghoul.toggleFullscreen")
    static let FullscreenModeKey = ghosttyToggleFullscreen.rawValue

    /// Notification sent to toggle split maximize/unmaximize.
    static let didToggleSplitZoom = Notification.Name("info.olaugh.aghoul.didToggleSplitZoom")

    /// Notification
    static let didReceiveInitialWindowFrame = Notification.Name("info.olaugh.aghoul.didReceiveInitialWindowFrame")
    static let FrameKey = "info.olaugh.aghoul.frame"

    /// Notification to render the inspector for a surface
    static let inspectorNeedsDisplay = Notification.Name("info.olaugh.aghoul.inspectorNeedsDisplay")

    /// Notification to show/hide the inspector
    static let didControlInspector = Notification.Name("info.olaugh.aghoul.didControlInspector")

    static let confirmClipboard = Notification.Name("info.olaugh.aghoul.confirmClipboard")
    ...
    static let didResizeSplit = Notification.Name("info.olaugh.aghoul.didResizeSplit")
    ...
    static let didEqualizeSplits = Notification.Name("info.olaugh.aghoul.didEqualizeSplits")

    /// Notification that renderer health changed
    static let didUpdateRendererHealth = Notification.Name("info.olaugh.aghoul.didUpdateRendererHealth")

    /// Notifications related to key sequences
    static let didContinueKeySequence = Notification.Name("info.olaugh.aghoul.didContinueKeySequence")
    static let didEndKeySequence = Notification.Name("info.olaugh.aghoul.didEndKeySequence")
    static let KeySequenceKey = didContinueKeySequence.rawValue + ".key"

    /// Notifications related to key tables
    static let didChangeKeyTable = Notification.Name("info.olaugh.aghoul.didChangeKeyTable")
